...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: qcm, error: erreurQcm } = await supabaseAdmin.from('annexe_qcms').select('*').eq('id', qcm_id).single();
    ...
    // Une seule tentative pour concours blanc
    ...
    const { data: questions, error: erreurQuestions } = await supabaseAdmin
      ...
      .select('id, annexe_items(id, est_correct)')
      .eq('qcm_id', qcm_id);
    ...
    const { error: erreurReponses } = await supabaseAdmin.from('annexe_attempt_answers').insert(
